    ...
    return (
      <div className="contentBox">
        <div className="container">
          <SearchBox>
            <FormField clas="light" type="text" placeholder="search favors..." onChange={(e)=>this.handleSearch(e)} value={search}/>
            ...
            {search && filterFavors ?
              <div className="favorsBox">
                <div className="favorsSearch">
                  {filterFavors.map(f => <FavorCard key={f._id} favorId={f._id} userId={f.creatorId._id} img={f.picturesUrls[0]} username={f.creatorId.username} date={f.creationdate} name={f.name} description={f.description}/>)}
                </div>
              </div>
              :
              <React.Fragment>
                <div className="favorsBox favorsOthers">
                  ...
                  <Slider {...settings}>
                    {favorsOthersNeed.map(f => <FavorCard key={f._id} favorId={f._id} userId={f.creatorId._id} img={f.picturesUrls[0]} username={f.creatorId.username} date={f.creationdate} name={f.name} description={f.description} slide/>)}
                  </Slider>
                </div>
                <div className="favorsBox">
                  <p className="title">Favors near you</p>
                  <div className="favorsNearby">
                    {favorsNearby.map(f => <FavorCard key={f._id} favorId={f._id} userId={f.creatorId._id} img={f.picturesUrls[0]} username={f.creatorId.username} date={f.creationdate} name={f.name} description={f.description} withBtns/>)}
                  </div>
                </div>
              </React.Fragment>
            }
