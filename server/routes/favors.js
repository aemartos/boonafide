...
const { uploadFavorPictures } = require('../config/cloudinary');
...
router.get('/search', isLoggedIn, (req, res, next) => {
  ...
  Favor.find({
    ...
    .then((favorsUnfiltered) => {
      const filterFavs = favorsUnfiltered.filter((f) => f.creatorId.username !== req.user.username && f.remainingFavNum > 0);
      const favors = filterFavs.map((f) => ({ ...JSON.parse(JSON.stringify(f)), creationdate: `${f.createdAt.getDate()}/${f.createdAt.getMonth() + 1}/${f.createdAt.getFullYear()}` }));
      res.json({ favors });
    })
    .catch((err) => next(err));
});
...
router.get('/offerFavors', isLoggedIn, (req, res, next) => {
  User.findById(req.user._id, { ...req.body.data, newUser: false })
    .then((user) => {
      ...
      Favor.find({
        type: 'Need',
        $or: categories.map((c) => ({ categories: c })),
